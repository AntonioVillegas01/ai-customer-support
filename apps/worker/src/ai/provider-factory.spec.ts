...
describe('createAiProviders', () => {
  ...
  it('uses the Gemini adapter for both llm and embeddings when AI_PROVIDER=gemini', () => {
    const providers = createAiProviders({ ...baseConfig, AI_PROVIDER: 'gemini', GEMINI_API_KEY: 'test-key' });
    expect(providers.llm).toBeInstanceOf(GeminiAdapter);
    expect(providers.embeddings).toBe(providers.llm);
  });
});
