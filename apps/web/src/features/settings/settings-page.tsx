...
export function SettingsPage() {
  ...
  return <div className="grid gap-4"><h1 className="text-2xl font-bold tracking-tight">{t('settings.title')}</h1><Card><h2 className="font-semibold">{t('settings.organization')}</h2><p className="text-sm text-muted-foreground">{t('settings.roleRestricted')}</p></Card><Card><h2 className="font-semibold">{t('settings.members')}</h2><p className="text-sm text-muted-foreground">{t('settings.roleRestricted')}</p></Card>{canManage ? <ApiKeysPanel /> : <Card><h2>{t('settings.apiKeys')}</h2><p>{t('settings.roleRestricted')}</p></Card>}{canManage ? <WidgetPanel /> : null}<AuditPanel /></div>;
}
...
function ApiKeysPanel() {
  ...
  return <Card><h2 className="font-semibold">{t('settings.apiKeys')}</h2><form className="mt-3 flex flex-wrap items-end gap-3" onSubmit={form.handleSubmit((values) => createMutation.mutate(values))}><Field label={t('settings.keyName')} error={form.formState.errors.name?.message}><TextInput {...form.register('name')} /></Field><Field label={t('settings.scopes')} error={form.formState.errors.scopesText?.message}><TextInput {...form.register('scopesText')} /></Field><Button type="submit" disabled={createMutation.isPending}>{t('settings.createKey')}</Button></form>{createMutation.isError ? <ErrorState message={createMutation.error.message} /> : null}{created === null ? null : <dialog open className="rounded-lg border border-border bg-card p-6 text-card-foreground shadow-lg"><h3 className="font-semibold">{t('settings.keyCreated')}</h3><code className="mt-3 block rounded-lg bg-muted p-3 font-mono text-xs">{created.key}</code><Button className="mt-3" type="button" onClick={() => setCreated(null)}>{t('common.close')}</Button></dialog>}{query.isLoading ? <LoadingState message={t('common.loading')} /> : null}{query.isError ? <ErrorState message={query.error.message} /> : null}<ul className="mt-4 grid gap-2">{query.data?.map((key) => <li className="flex items-center justify-between rounded-lg border border-border p-3" key={key.id}><span>{key.name} · {key.prefix}</span><Button type="button" variant="danger" disabled={key.revokedAt !== null} onClick={() => void revokeApiKey(orgId, key.id).then(() => queryClient.invalidateQueries({ queryKey: ['api-keys', orgId] }))}>{t('common.delete')}</Button></li>)}</ul></Card>;
}
