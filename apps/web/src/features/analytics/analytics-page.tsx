...
export function AnalyticsPage() {
  ...
  return <div className="grid gap-4"><h1 className="text-2xl font-bold">{t('analytics.title')}</h1>{query.isLoading ? <LoadingState message={t('common.loading')} /> : null}{query.isError ? <ErrorState message={t('analytics.unavailable')} /> : null}{query.data === undefined ? null : <div className="grid gap-4 md:grid-cols-4"><Metric label="Conversations" value={query.data.conversations ?? 0} /><Metric label="AI runs" value={query.data.aiRuns ?? 0} /><Metric label="Tokens" value={query.data.tokens ?? 0} /><Metric label="Cost" value={query.data.aiCostUsd ?? '0'} /></div>}</div>;
}
function Metric({ label, value }: Readonly<{ label: string; value: string | number }>) { return <Card><p className="text-sm text-slate-600">{label}</p><strong className="text-2xl">{value}</strong></Card>; }
