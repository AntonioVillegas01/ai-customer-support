...
export function KnowledgePage() {
  ...
  return <div className="grid gap-4"><h1 className="text-2xl font-bold">{t('knowledge.title')}</h1><div className="grid gap-4 lg:grid-cols-3"><FaqForm onChanged={refresh} /><UrlForm onChanged={refresh} /><FileForm onChanged={refresh} /></div><SearchBox /><Card>{query.isLoading ? <LoadingState message={t('common.loading')} /> : null}{query.isError ? <ErrorState message={query.error.message} /> : null}{query.data?.length === 0 ? <EmptyState message={t('common.empty')} /> : null}<ul className="grid gap-3">{query.data?.map((source) => <li key={source.id} className="flex flex-wrap items-center justify-between gap-3 rounded border border-slate-200 p-3"><div><strong>{source.name}</strong><div className="mt-1 flex gap-2"><Badge value={source.type} /><Badge value={source.latestIngestionStatus ?? 'pending'} /></div></div><div className="flex gap-2"><Button type="button" variant="secondary" onClick={() => void reprocessSource(orgId, source.id).then(refresh)}>{t('common.reprocess')}</Button><Button type="button" variant="danger" onClick={() => void deleteSource(orgId, source.id).then(refresh)}>{t('common.delete')}</Button></div></li>)}</ul></Card></div>;
}
